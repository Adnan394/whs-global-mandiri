    ...
    <div class="hero-container mb-5">
        <div class="rounded-4">
            <div class="d-flex justify-content-center">
                <h2 class="text-center mb-4 title-underline mb-3">My Profile</h2>
            </div>
            <form action="{{ route('crewing.update.profile') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row d-flex gap-3 justify-content-center">
                    <div class="col-11 col-md-3 bg-white shadow p-3 rounded-4">
                        ...
                        <hr>
                        <h5 class="mb-2 fw-bold">Name : {{ $user->name }}</h5>
                        <p class="mb-1">Role : {{ $user->role }}</p>
                        @if($user->email)
                            <p class="mb-1">Email : {{ $user->email }}</p>
                        @endif
                        @if($crew->phone)
                            <p class="mb-1">Phone : {{ $crew->phone }}</p>
                        @endif
                    </div>
                    <div class="col-11 col-md-8 bg-white shadow p-4 rounded-4">
                        ...
                        <!--        <small class="text-danger" style="font-size: 12px">(KTP, KK, Birth Certificate) *Make 1 File in Pdf Format</small>-->
                        <!--    </div> --}}-->
                        <!--    <div class="col mb-3">-->
                        ...
                        <div class="row w-100">
                            <div class="col mb-3">
                                <label for="smc" class="form-label">Card ID</label>
                                <input type="file" name="card_id" class="form-control mb-2" id="card_id"accept="image/jpeg">
                                <small class="text-danger" style="font-size: 12px">(Card ID) *Jpg Format</small>
                            </div>
                        </div>
